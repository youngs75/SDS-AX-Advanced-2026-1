...
def _run_in_process(query: str) -> str:
    """CodingAssistantAgent를 in-process로 직접 실행한다.

    CodingConfig.get_model()이 OPENAI_API_KEY 환경변수를 읽어
    OpenAI API를 직접 호출한다 (OpenRouter 불필요).
    """
    ...
    # async event loop 안에서 호출될 수 있으므로 (Langfuse run_experiment 등)
    try:
    ...
    result = asyncio.run(agent.graph.ainvoke(invoke_input))
