...
class TestGEvalMetrics:
    ...
    def test_response_completeness_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
    ...
    def test_citation_quality_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            mock_model_fn.return_value = MagicMock(spec=DeepEvalBaseLLM)
            from src.loop2_evaluation.custom_metrics import create_citation_quality_metric
    ...
    def test_bias_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            mock_model_fn.return_value = MagicMock(spec=DeepEvalBaseLLM)
            from src.loop2_evaluation.custom_metrics import create_bias_metric
    ...
    def test_toxicity_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            mock_model_fn.return_value = MagicMock(spec=DeepEvalBaseLLM)
            from src.loop2_evaluation.custom_metrics import create_toxicity_metric
    ...
    def test_pii_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            mock_model_fn.return_value = MagicMock(spec=DeepEvalBaseLLM)
            from src.loop2_evaluation.custom_metrics import create_pii_metric
    ...
    def test_disclaimer_creation(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            mock_model_fn.return_value = MagicMock(spec=DeepEvalBaseLLM)
            from src.loop2_evaluation.custom_metrics import create_disclaimer_metric
...
class TestSafetyMetric:
    def test_safe_response(self):
        with patch("src.loop2_evaluation.custom_metrics.get_deepeval_model") as mock_model_fn:
            ...
            from src.loop2_evaluation.custom_metrics import SafetyMetric
