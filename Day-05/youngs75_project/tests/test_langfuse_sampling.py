...
from src.loop2_evaluation.batch_evaluator import (
    _run_metrics_on_testcase,
    batch_evaluate_langfuse,
    monitor_langfuse_scores,
)
from src.loop2_evaluation.langfuse_bridge import fetch_traces
...
def test_fetch_traces_applies_sampling_and_skip_prefix(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.loop2_evaluation.langfuse_bridge.enabled",
        lambda _settings: True,
    )
    monkeypatch.setattr(
        "src.loop2_evaluation.langfuse_bridge.client",
...
def test_batch_evaluate_langfuse_forwards_sampling_options(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.loop2_evaluation.batch_evaluator.fetch_traces",
        fake_fetch_traces,
    )
