...
Use `init_chat_model()` for provider-agnostic model creation. This is the recommended way to instantiate models when you need runtime configurability.
...
# "openai" | "anthropic" | "google-genai" | "bedrock" | "ollama" | "fireworks" | "together" | "mistralai"
```

### Runtime-Configurable Model

Omit the model argument to make it configurable at runtime via `RunnableConfig`:

```python
# No model specified — configurable at runtime
...
|----------|----------|------|
| Prompt patterns | Design structured prompts with XML, variables, few-shot | `assets/prompt-patterns/` |
| Message patterns | Construct all 5 message types, multimodal, RemoveMessage | `assets/message-patterns/` |
| Tool definition | @tool decorator, Pydantic args_schema, InjectedToolArg | `assets/tool-definition/` |
| Runnable patterns | LCEL composition, RunnableLambda, RunnableParallel | `assets/runnable-patterns/` |
| Tool runtime | Access state, store, stream_writer from within tools | `assets/tool-runtime-example/` |
| Structured output | .with_structured_output(), JSON mode, Pydantic schemas | `assets/structured-output-example/` |
| Streaming | Multi-mode streaming, token-level output | `assets/streaming-example/` |
| Memory | Short-term and long-term memory patterns | `assets/memory-example/` |

## Quick Checklist

Before finalizing your implementation:

- [ ] Using create_agent (not legacy AgentExecutor)?
- [ ] Tools defined with @tool decorator and proper type hints?
- [ ] Model instantiated via init_chat_model() or provider package?
- [ ] Structured output using .with_structured_output() where needed?
- [ ] Error handling covers tool failures and model errors?
- [ ] Streaming configured if real-time output needed?
