...
LangChain v0.3+ and LangGraph v1.0 deprecated many legacy patterns. Key rules:

1. **10 Banned APIs**: `create_react_agent`, `AgentExecutor`, `initialize_agent`, `LLMChain`, `ConversationChain`, `SequentialChain`, `load_tools`, `ConversationBufferMemory`, `ConversationSummaryMemory` — all replaced by `create_agent` + checkpointer
2. **Provider imports**: All model classes moved to dedicated `langchain-*` packages (e.g., `from langchain_openai import ChatOpenAI`)
3. **Memory → Checkpointer**: `ConversationBufferMemory()` → `InMemorySaver()` passed to `create_agent`

For the full ban list, replacement map, and verification `rg` commands, see `references/deprecations-and-migration.md`.
...
| Topic | Reference |
|-------|-----------|
| Full deprecation ban list + migration commands | `references/deprecations-and-migration.md` |
...
- [ ] All langchain-* provider packages installed?
- [ ] Python version ≥ 3.11?
- [ ] No banned legacy APIs in codebase (run verification rg commands)?
- [ ] Provider imports from dedicated packages, not monolithic langchain?
- [ ] Environment variables configured for chosen providers?
- [ ] uv.lock / requirements.txt up to date?
