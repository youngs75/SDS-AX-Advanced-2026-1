...
When parallel branches each call `interrupt()`, resume all of them in a single invocation by mapping each interrupt's `.id` to its resume value:

```python
resume_map = {i.id: resolve(i.value) for i in result["__interrupt__"]}
graph.invoke(Command(resume=resume_map), config)
```

For detailed parallel interrupt examples (Python + TypeScript), resume map patterns, and subgraph interrupt re-execution behavior, see `references/advanced-interrupt-patterns.md`.
...
| Topic | Reference |
|-------|-----------|
| Parallel interrupts, resume map, subgraph re-execution | `references/advanced-interrupt-patterns.md` |
...
- Checkpointer attached to graph?
- Thread ID provided for all invoke/stream calls?
- interrupt() payload is JSON-serializable?
- Code before interrupt() is idempotent?
- Using Command(resume=...) to resume (not plain dict)?
- Subgraph interrupt chains have idempotent parent nodes?
