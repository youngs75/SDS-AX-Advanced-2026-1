...
| Topic | Reference |
|-------|-----------|
| 8-layer middleware stack, Summarization, PatchToolCalls, PromptCaching | `references/harness-architecture.md` |
| 5 leverage points, 4 failure modes, tool context pollution | `references/context-engineering.md` |
| 4-layer defense model, credential management, deployment checklist | `references/security.md` |
...
|----------|----------|------|
| Quickstart | Bootstrap create_deep_agent() project | `assets/quickstart-example/` |
| Middleware pipeline | Custom middleware with state extension | `assets/middleware-pipeline/` |
| Built-in tools | Filesystem, todo, large result handling | `assets/built-in-tools/` |
| Sandbox execution | Custom sandbox backend (BaseSandbox) | `assets/sandbox-execution/` |
| Context engineering | 5-layer system prompt assembly | `assets/context-engineering/` |
| Non-interactive | Scripting mode, CI/CD, auto-approve | `assets/non-interactive-execution/` |
| Summarization | Conversation offloading, history backup | `assets/summarization-strategy/` |
| Data analysis | CSV analysis with sandboxed execution | `assets/data-analysis/` |
| Skill loading | SKILL.md progressive disclosure | `assets/skill-loading/` |

---

## Quick Checklist

- Using create_deep_agent() (not raw LangGraph for agent tasks)?
- System prompt follows 5-layer assembly pattern?
- Tool definitions follow least-privilege principle?
- Sandbox configured for code execution tasks?
- Context window managed (summarization strategy defined)?
- Security: no secrets in system prompt, tools sandboxed?
