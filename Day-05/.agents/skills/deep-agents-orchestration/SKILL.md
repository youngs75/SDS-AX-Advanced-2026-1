...
|----------|----------|------|
| Subagent delegation | Specialized sub-agents with delegation contracts | `assets/subagent-delegation/` |
| HITL approval | Interrupt-based approval for risky operations | `assets/hitl-approval/` |

---

## Quick Checklist

- Delegation contracts define clear boundaries per sub-agent?
- Sub-agents have least-privilege tool access?
- HITL approval configured for destructive/write operations?
- Context isolation prevents cross-agent data leaks?
- Orchestrator handles sub-agent failures gracefully?
