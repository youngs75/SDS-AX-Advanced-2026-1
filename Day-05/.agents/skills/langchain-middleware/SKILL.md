...
LangChain provides input/output guardrail patterns for safety enforcement:

- **Input guardrails**: Validate/sanitize user input before model processing (PII detection, prompt injection defense)
- **Output guardrails**: Filter/validate model responses (content policy, format enforcement)
- **Layered defense**: Combine middleware-level guardrails with model-level safety settings

For detailed PIIMiddleware implementation, hierarchical defense patterns, and production guardrail configuration, see `references/guardrails.md`.

## MCP Integration

LangChain supports Model Context Protocol (MCP) for standardized tool connectivity:

- **MultiServerMCPClient**: Connect to multiple MCP servers simultaneously
- **3 transport types**: stdio (local processes), SSE (HTTP streaming), Streamable HTTP
- **Tool interceptors**: Modify/filter tools before they reach the model
- **FastMCP**: Build custom MCP servers exposing LangChain tools

For MultiServerMCPClient setup, transport configuration, and tool interceptor patterns, see `references/mcp-integration.md`.

## Deep-Dive References

| Topic | Reference |
|-------|-----------|
| PIIMiddleware, input/output guardrails, layered defense | `references/guardrails.md` |
| MultiServerMCPClient, transports, tool interceptors, FastMCP | `references/mcp-integration.md` |
...
|----------|----------|------|
| Custom middleware | Build middleware with state extension, prompt injection | `assets/middleware-example/` |
| MCP client | Connect to MCP servers with multi-transport | `assets/mcp-client-example/` |
| Guardrails | Input/output validation, PII filtering | `assets/guardrails-example/` |

## Quick Checklist

Before finalizing your implementation:

- [ ] Middleware registered in correct order (outermost first)?
- [ ] Guardrails covering both input and output?
- [ ] MCP connections using appropriate transport type?
- [ ] Error handling in middleware doesn't swallow exceptions?
- [ ] Middleware tested with both streaming and non-streaming?
