...
|----------|----------|------|
| Backend patterns | CompositeBackend with path-based routing | `assets/backend-patterns/` |
| Memory strategy | Cross-thread memory with StoreBackend namespaces | `assets/memory-strategy/` |

---

## Quick Checklist

- Backend type matches use case (ephemeral vs persistent vs composite)?
- Namespace hierarchy is consistent and documented?
- Cross-thread memory uses Store with proper namespace isolation?
- Memory lifecycle defined (TTL, manual cleanup)?
- Sensitive data excluded from long-term storage?
