"""DeepEval 전용 LLM 모델 래퍼.

DeepEval 라이브러리의 Synthesizer, Metric 등은 model= 인자로
DeepEvalBaseLLM 인스턴스를 받습니다. 이 모듈은 OpenRouter 또는
OpenAI direct 클라이언트를 DeepEval이 이해할 수 있는 형태로 감쌉니다.

아키텍처:
    Provider API ← OpenAI SDK ← OpenRouterModel(DeepEvalBaseLLM)
                              ↑
...
from src.llm.openrouter import get_chat_model_name, get_llm_client, get_llm_provider
...
class OpenRouterModel(DeepEvalBaseLLM):
    ...
    내부적으로 OpenAI SDK를 사용하여 provider별 LLM을 호출합니다.

    Args:
        model_name: 사용할 모델명. None이면 provider 기본값 사용
        api_key: provider API 키 override. None이면 settings에서 읽음
    """

    def __init__(self, model_name: str | None = None, api_key: str | None = None):
        settings = get_settings()
        self._provider = get_llm_provider(settings)
        self._model_name = model_name or get_chat_model_name(settings)
...
def get_deepeval_model(model_name: str | None = None) -> OpenRouterModel:
    """DeepEval 메트릭/합성기에 전달할 provider별 모델 인스턴스를 생성합니다.
