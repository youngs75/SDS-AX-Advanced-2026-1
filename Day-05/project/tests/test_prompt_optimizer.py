...
from src.loop2_evaluation.calibration_cases import CalibrationCase
from src.loop2_evaluation.prompt_optimizer import (
    CaseScore,
    PromptEvaluation,
...
def test_evaluate_prompt_with_mocked_metric(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.loop2_evaluation.prompt_optimizer.cases_for_metric",
        lambda metric, max_cases=None: sample_cases,
    )
    monkeypatch.setattr(
        "src.loop2_evaluation.prompt_optimizer._build_metric",
...
def test_optimize_metric_prompt_accepts_better_candidate(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.loop2_evaluation.prompt_optimizer.evaluate_prompt",
        fake_evaluate_prompt,
    )
    monkeypatch.setattr(
        "src.loop2_evaluation.prompt_optimizer._request_prompt_update",
        lambda **kwargs: {
...
def test_optimize_all_prompts_forwards_langfuse_hints(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.loop2_evaluation.prompt_optimizer.optimize_metric_prompt",
